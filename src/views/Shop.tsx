...
export const Shop: React.FC<ShopProps> = ({ onViewChange, onSelectProduct }) => {
  ...
  // Set initial max price limit once products load
  useEffect(() => {
    if (products && products.length > 0) {
      const highestPrice = Math.max(...products.map(p => p.price));
      const fallbackPrice = highestPrice > 0 ? highestPrice : 12000;
      setMaxPriceLimit(fallbackPrice);
      setMaxPrice(fallbackPrice);
    }
  }, [products]);

  // OPTIMIZATION 1: useMemo se instantaneous pipeline client-side filtering without re-render lags
    ...
    <div className="bg-[#F7E7CE] text-[#1C1A17] min-h-screen pb-20 selection:bg-[#BF953F]/30 selection:text-[#1C1A17]">
      ...
      <div className="max-w-[1400px] mx-auto px-3 sm:px-6 lg:px-8 py-6 sm:py-14">
        ...
        <div className="lg:hidden flex flex-col gap-4 mb-6">
          <div className="flex gap-2 overflow-x-auto hide-scrollbar pb-2">
            {(['all', 'men', 'women', 'kids'] as const).map((cat) => (
              <button
                ...
                className={`px-4 py-2 sm:px-5 sm:py-2.5 rounded-full text-[9px] sm:text-[10px] font-bold tracking-[0.2em] uppercase transition-colors duration-200 cursor-pointer whitespace-nowrap flex-shrink-0 border ${
                  activeCategory === cat
                    ? 'bg-[#1C1A17] text-[#BF953F] border-[#1C1A17] shadow-sm'
                    : 'bg-[#FFFFFF] text-[#59534E] border-[#E3DDD3]'
                }`}
              >
                {cat === 'all' ? 'All' : `${cat}'s`}
              </button>
          ...
          <div className="flex justify-between items-center gap-2 sm:gap-3">
            <button 
              ...
              className="flex-1 flex items-center justify-center gap-1.5 sm:gap-2 bg-[#FFFFFF] border border-[#E3DDD3] rounded-full py-2.5 sm:py-3 px-4 sm:px-6 text-[9px] sm:text-[10px] font-bold tracking-widest text-[#1C1A17] transition-colors shadow-sm group active:bg-[#BF953F] active:text-white"
            >
              <Filter className="w-3.5 h-3.5 text-[#B58A3D] group-active:text-white" />
              REFINE
            </button>
            ...
            <div className="relative flex-1">
              <select
                ...
                className="w-full appearance-none bg-[#FFFFFF] text-[#1C1A17] text-[9px] sm:text-[10px] font-bold tracking-widest border border-[#E3DDD3] rounded-full py-2.5 sm:py-3 px-4 sm:px-6 outline-none cursor-pointer shadow-sm"
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-10 items-start">
          ...
          <AnimatePresence>
            {isMobileFilterOpen && (
              <motion.div 
                ...
                className="fixed inset-0 bg-black/60 backdrop-blur-sm z-40 lg:hidden transform-gpu"
              />
            )}
          </AnimatePresence>
          ...
          {/* SIDEBAR FILTERS */}
          <aside className={`
            bg-[#FFFFFF] text-left transition-transform duration-300 ease-in-out z-50 transform-gpu will-change-transform
            ${isMobileFilterOpen 
              ? 'fixed inset-x-0 bottom-0 rounded-t-[2.5rem] p-6 shadow-[0_-10px_40px_rgba(0,0,0,0.15)] max-h-[85vh] overflow-y-auto translate-y-0' 
          ...
          `}>
            {isMobileFilterOpen && (
              <div className="flex flex-col items-center mb-6 lg:hidden">
                ...
                <div className="w-full flex items-center justify-between">
                  ...
                  <button onClick={() => setIsMobileFilterOpen(false)} className="p-2 bg-[#F5F5F5] rounded-full text-[#1C1A17]">
                    <X className="w-4 h-4" />
                  </button>
                </div>
              </div>
                ...
                {['S', 'M', 'L', 'XL', '50ml', '100ml', '41', '42', '43'].map((size) => {
                  ...
                  return (
                    <button
                      ...
                      className={`py-2 px-3 text-[10px] font-mono font-bold border rounded-lg transition-colors duration-150 cursor-pointer ${
                        isChecked 
                          ? 'bg-[#1C1A17] text-[#BF953F] border-[#1C1A17] shadow-sm' 
                          : 'border-[#E3DDD3] text-[#59534E] bg-[#F5F5F5]'
                      }`}
                    >
                      {size}
                    </button>
                ...
                {['black', 'white', 'blue', 'dark blue', 'gray', 'cream', 'maroon', 'olive', 'beige', 'navy', 'peach'].map((col) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-2 text-[9px] font-bold uppercase px-3 py-2 border rounded-xl transition-all duration-150 cursor-pointer ${
                        isChecked 
                          ? 'bg-[#F5F5F5] text-[#1C1A17] border-[#BF953F] font-extrabold shadow-sm ring-1 ring-[#BF953F]' 
                          : 'border-[#E3DDD3] text-[#59534E] bg-[#FFFFFF]'
                      }`}
                    >
                      <span 
            ...
            {isMobileFilterOpen && (
              <button 
                onClick={() => setIsMobileFilterOpen(false)}
                className="w-full mt-6 py-4 bg-[#1C1A17] text-[#BF953F] text-[10px] font-bold tracking-[0.25em] uppercase rounded-xl"
              >
          ...
          <main className="lg:col-span-9 flex flex-col gap-6">
            ...
            <div className="hidden lg:flex flex-row justify-between items-center gap-4 bg-[#FFFFFF] border border-[#E3DDD3] p-4 rounded-xl shadow-[0_4px_20px_rgba(28,26,23,0.02)]">
              <div className="flex gap-1.5">
                {(['all', 'men', 'women', 'kids'] as const).map((cat) => (
                  <button
                    key={cat}
                    onClick={() => setActiveCategory(cat)}
                    className={`px-5 py-2.5 rounded-lg text-[10px] font-bold tracking-[0.2em] uppercase transition-all duration-200 cursor-pointer border-0 ${
                      activeCategory === cat
              ...
              <div className="flex items-center gap-2.5">
                ...
                <select
                  value={sortBy}
                  onChange={(e) => setSortBy(e.target.value as any)}
                  className="bg-[#F5F5F5] text-[#1C1A17] text-[10px] font-bold tracking-widest border border-[#E3DDD3] rounded-lg py-2.5 px-4 outline-none cursor-pointer"
                >
            ...
            {/* OPTIMIZATION 2: Remounting lag aur heavy DOM unmounting thrashing ko fix kar diya */}
            {loading ? (
              ...
              <div className="grid grid-cols-3 sm:grid-cols-2 xl:grid-cols-3 gap-2 sm:gap-8 transform-gpu">
              ...
              <div className="border border-[#E3DDD3] bg-[#FFFFFF] text-center py-24 rounded-2xl flex flex-col items-center justify-center gap-4">
                ...
                <button 
                  onClick={clearAllFilters}
                  className="px-6 py-3 bg-[#1C1A17] text-[#BF953F] text-[10px] uppercase font-bold tracking-widest rounded-xl mt-2 cursor-pointer"
                >
                  RESET ALL FILTERS
                </button>
              </div>
        ...
        {quickViewProduct && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 transform-gpu">
            ...
            <motion.div 
              ...
              className="relative max-w-3xl w-full bg-[#FFFFFF] border border-[#E3DDD3] rounded-2xl p-6 sm:p-8 shadow-2xl overflow-y-auto max-h-[90vh] grid grid-cols-1 md:grid-cols-12 gap-8 text-left will-change-transform"
            >
              <button 
                onClick={() => setQuickViewProduct(null)}
                className="absolute top-4 right-4 w-8 h-8 rounded-full border border-[#E3DDD3] bg-[#F5F5F5] text-[#1C1A17]/70 flex items-center justify-center hover:bg-[#1C1A17] hover:text-[#BF953F] transition-colors cursor-pointer z-10"
              >
                <X className="w-4 h-4" />
              </button>
              ...
              <div className="md:col-span-5">
                <div className="aspect-[3/4] rounded-xl overflow-hidden bg-[#F5F5F5] border border-[#E3DDD3]">
                  <img src={quickViewProduct.image} alt={quickViewProduct.name} className="w-full h-full object-cover object-top" loading="lazy" />
                </div>
              </div>
              ...
              <div className="md:col-span-7 flex flex-col justify-center">
                ...
                <div className="flex gap-3 mt-auto">
                  <button
                    ...
                    className="flex-1 py-3.5 bg-[#1C1A17] text-[#BF953F] font-bold tracking-widest text-[10px] uppercase rounded-xl text-center cursor-pointer"
                  >
                    VIEW SPECIFICS
                  </button>
                  <button
                    ...
                    className="py-3.5 px-6 border border-[#E3DDD3] bg-[#F5F5F5] text-[#B58A3D] rounded-xl flex items-center justify-center cursor-pointer"
                  >
...
// OPTIMIZATION 3: React.memo wrapper taake unneeded cards re-render na hon jab filter badle
const ProductCard: React.FC<CardProps> = React.memo(({ 
  ...
  // OPTIMIZATION 4: Heavy local useEffect states ko derived metrics me tabdeel kar diya thread load hatane ke liye
  const primaryColor = product.colors && product.colors.length > 0 ? product.colors[0] : 'cream';
  const isFavorited = wishlist.includes(product.id);
  
  const secondImage = useMemo(() => {
    if (product.gallery && product.gallery.length > 1) return product.gallery[1];
    if (product.colorImagesList && product.colorImagesList.length > 1) return product.colorImagesList[1].imgUrl;
    return null;
  }, [product.gallery, product.colorImagesList]);
  ...
  return (
    <div 
      ...
      // OPTIMIZATION 5: transition-all hata kar GPU layered structural transforms set kardiye hain layers smoothness k liye
      className="bg-[#FFFFFF] border border-[#E3DDD3] rounded-md sm:rounded-xl overflow-hidden shadow-sm transition-[border-color,box-shadow] duration-300 cursor-pointer flex flex-col h-full group relative text-left transform-gpu will-change-transform"
    >
      <div className="relative aspect-[3/4] overflow-hidden bg-[#F5F5F5]">
        <img 
          src={product.image} 
          alt={product.name} 
          // OPTIMIZATION 6: Transition targeted transform parameters ONLY
          className={`w-full h-full object-cover transform scale-100 sm:group-hover:scale-105 transition-all duration-500 object-top absolute inset-0 ${secondImage ? 'sm:group-hover:opacity-0' : ''}`}
          loading="lazy"
        />
        ...
        {secondImage && (
          <img 
            ...
            className="w-full h-full object-cover transform scale-100 sm:group-hover:scale-105 transition-opacity duration-500 object-top absolute inset-0 opacity-0 sm:group-hover:opacity-100"
            loading="lazy"
          />
        )}
        ...
        <button 
          ...
          className="absolute top-1.5 right-1.5 sm:top-4 sm:right-4 z-10 w-6 h-6 sm:w-9 sm:h-9 rounded-full bg-[#FFFFFF]/80 backdrop-blur-md border border-[#E3DDD3] flex items-center justify-center transition-colors shadow-sm"
        >
          <Heart className={`w-3 h-3 sm:w-4 sm:h-4 ${isFavorited ? 'fill-red-500 text-red-500 scale-105' : 'text-[#59534E]'}`} />
        </button>
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-[#1C1A17]/40 to-transparent opacity-0 sm:group-hover:opacity-100 transition-opacity duration-300 hidden sm:flex flex-col items-center justify-end p-5">
          <button 
            onClick={(e) => { e.stopPropagation(); onQuickView(product); }}
            className="w-full py-3 text-center bg-[#FFFFFF] text-[#1C1A17] text-[9px] font-bold tracking-widest uppercase rounded shadow-lg transform translate-y-4 sm:group-hover:translate-y-0 transition-transform duration-300"
          >
      ...
      <div className="p-2 sm:p-5 flex flex-col flex-1 text-left relative mt-1 sm:mt-0">
        ...
        <h3 className="font-serif text-[10px] sm:text-base font-normal tracking-wide text-[#1C1A17] sm:group-hover:text-[#B58A3D] transition-colors mb-1 sm:mb-2 leading-tight sm:leading-snug line-clamp-1">
          {product.name}
        </h3>
        ...
        <button
          onClick={(e) => {
            e.stopPropagation();
            addToCart(product, product.sizes[0] || 'M', primaryColor);
          }}
          className="w-full mt-auto py-1.5 sm:py-3.5 rounded-md sm:rounded-xl bg-[#F5F5F5] border border-[#E3DDD3] text-[7px] sm:text-[9px] font-bold tracking-[0.1em] sm:tracking-[0.25em] text-[#1C1A17] transition-colors uppercase cursor-pointer flex items-center justify-center gap-1 sm:gap-2"
        >
          <ShoppingCart className="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5 text-[#B58A3D]" /> 
